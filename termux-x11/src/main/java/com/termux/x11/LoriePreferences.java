...
private void performRestartSequence() {
   ...
   // Try each command separately
    ...
    Toast.makeText(this, "Wine processes stopped", Toast.LENGTH_SHORT).show();
}
...
if (p.getKey().contentEquals("wine_settings")) {
    if (loriePreferences != null && loriePreferences.isActivityValid()) {
        // Use reflection to call XoDosWizard
        try {
            Class<?> xoDosWizardClass = Class.forName("com.termux.app.XoDosWizard");
            Object xoDosWizardInstance = xoDosWizardClass
                .getDeclaredConstructor(Activity.class)
                .newInstance(loriePreferences);
            Method startMethod = xoDosWizardClass.getMethod("start");
            startMethod.invoke(xoDosWizardInstance);
        } catch (Exception e) {
            e.printStackTrace();
            Toast.makeText(loriePreferences, 
                "Failed to open Wine settings: " + e.getMessage(), 
                Toast.LENGTH_LONG).show();
        }
    }
    return true;
            ...
            /*    // Add Wine Stop handler
    if (p.getKey().contentEquals("wine_stop")) {
        if (loriePreferences != null && loriePreferences.isActivityValid()) {
            new AlertDialog.Builder(requireContext())
                .setTitle("Stop Wine Desktop")
                .setMessage("Are you sure you want to stop and exit Wine desktop?")
                .setPositiveButton(android.R.string.ok, (dialog, which) -> {
                    loriePreferences.performRestartSequence();
                    Toast.makeText(loriePreferences, "Stopping Wine desktop...", Toast.LENGTH_SHORT).show();
                })
